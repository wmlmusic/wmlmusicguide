...
<div id="content">
	...
	<!--  start page-heading -->
	<div id="page-heading">
		<h1><?php echo ucwords($data['formStatus']) ?> Artist</h1>
	</div>
	<!-- end page-heading -->
			...
			<div id="table-content" style="border:0px solid red">		
				...
				<form id="artistForm" action="artistcontroller.php" method="post" enctype="multipart/form-data">
					...
					<tr>
						...
						<td><input type="text" class="inp-form required" name="artist_address" value="<?php if(isset($data['row']['address'])) echo $data['row']['address']; ?>" /></td>
						<th>City:</th>
						<td><input type="text" class="inp-form required" name="artist_city" value="<?php if(isset($data['row']['city'])) echo $data['row']['city']; ?>" /></td>
					</tr>
					<tr>
						<th valign="top">State:</th>
						<td><input type="text" class="inp-form required" name="artist_state" value="<?php if(isset($data['row']['state'])) echo $data['row']['state']; ?>" /></td>
						<th>Zip Code:</th>
						<td><input type="text" class="inp-form" name="artist_zip" value="<?php if(isset($data['row']['zipcode'])) echo $data['row']['zipcode']; ?>" /></td>
					</tr>
					<tr>
						<th valign="top">Country:</th>
						<td><input type="text" class="inp-form required" name="artist_country" value="<?php if(isset($data['row']['country'])) echo $data['row']['country']; ?>" /></td>
						<th>Phone:</th>
						<td><input type="text" class="inp-form" name="artist_phone" value="<?php if(isset($data['row']['phone'])) echo $data['row']['phone']; ?>" /></td>
					</tr>					
					<tr>
						<th valign="top">Email:</th>
						<td><input type="email" class="inp-form required" name="artist_email" value="<?php if(isset($data['row']['email'])) echo $data['row']['email']; ?>" /></td>
						<th>Website:</th>
					...
					<tr>
						...
						<td>
							<select name="artist_company" class="required">
					...
					<tr>
						...
						<th>Artist Type</th>
						<td>
							<select name="artist_type" class="required">
					...
					<tr>
						...
						<td>
							<select name="artist_genre[]" class="chosen-select required" data-placeholder="Choose a Genre..." style="width:350px;" multiple>
								<?php echo $data['genre']; ?>
							</select>
						</td>
						<th>Field</th>
					...
					<tr>
						...
						<td><input type="file" name="artist_cover" accept="image/*" /></td>
						<th>Picture:</th>
						<td><input type="file" name="artist_picture" accept="image/*" /></td>
					</tr>
					<tr>
						<th valign="top">&nbsp;</th>
						<td>&nbsp;</td>
					...
					<tr>
						...
						<td>
							<!-- <textarea class="form-textarea" name="youtube_channel" cols="" rows=""><?php if(isset($data['row']['youtube_channel'])) echo $data['row']['youtube_channel']; ?></textarea> -->
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$('#artistForm').validate({errorElement: "div"});
