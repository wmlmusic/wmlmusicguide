<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if(empty($_POST['id'])){
			...
			if($insert){
				if(isset($_FILES)){
					$createImg = saveResizeImage($_FILES['category_image'], $insert, 'category');
				}
				//echo print_r('mp_type');$_POST['mp_type'];exit;
			...
			if($update || isset($_FILES)){
				if(isset($_FILES)){
					$createImg = saveResizeImage($_FILES['category_image'], $arrData['mp_id'], 'category');				
				}
