...
(function ($) {
	$.accordionSlider = function (wrapper, settings) {
		...
		var controls_play_url = 'dcodes/sliders/dream-accordion/data/icons/controls_play.gif';
		var controls_pause_url = 'dcodes/sliders/dream-accordion/data/icons/controls_pause.gif';
		var playBtnUrl = 'dcodes/sliders/dream-accordion/data/icons/play2.png';
		if (useControls) {
		...
		function setupDone() {
			...
			sliderCointainer.stop().animate({
				'opacity': 1
			}, {
				duration: 500,
				easing: "easeOutSine"
			});
			if (useControls) controls.animate({
				opacity: 1
			}, {
				duration: 500,
				easing: "easeOutSine"
		...
		function updateSlidesViaTrack() {
			...
			if (!keepSelection && counter == -1) {
				...
				sliderHolder.stop().animate(prop, {
					duration: 350,
					easing: 'easeOutSine'
				});
				//
	...
	*/
	...
	function showScroll(){
		...
		function hidePreviousSlideData(id) { //hide vertical title and captions
			...
			if ($(slideArr[id]).data('imgCopy')) $(slideArr[id]).data('imgCopy').stop().animate({
				'opacity': 1
			}, {
				duration: 1000
			});
		...
		function openSlide(j, updateContent) {
			...
			if (!componentInited) return;
			if ($(slideArr[j]).data('imgCopy')) $(slideArr[j]).data('imgCopy').stop().animate({
				'opacity': 0
			}, {
				duration: 1000
			});
			...
			captionStartTimeoutID = setTimeout(function () {
				checkCaptions(j)
			}, transitionTime - 250);
		}
		...
		function distributeSpace(tween) {
			...
			for (i; i < playlistLength; i++) {
				...
				prop[animProp] = newX + 'px';
				item.stop().animate(prop, {
					duration: tween ? transitionTime : 0,
					easing: transitionEase
				});
				item[0].opened = false;
			}
			...
			if ($(slideArr[id]).data('captions') != undefined) {
				...
				for (i; i < len; i++) {
					...
					if (i != len - 1) {
						...
						$(caption).delay(captionOpenDelay * i).animate({
							width: cfw,
							left: cof
						}, {
							duration: captionToggleSpeed,
		...
		function createCloseVideoBtn() {
			//close btn
			var closeTextUrl = 'dcodes/sliders/dream-accordion/data/icons/close.png';
			...
			closeText.load(function () {}).attr('src', closeTextUrl);
